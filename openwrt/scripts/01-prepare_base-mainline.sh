...
git clone https://$github/jianyun8023/target_linux_rockchip-6.x target/linux/rockchip -b openwrt-24.10
...
release_kernel_version=$(curl -sL https://raw.githubusercontent.com//0118Add/OpenWrt-Actions/master/tags/kernel-6.12 | sed -n 's/^LINUX_KERNEL_HASH-\([0-9.]\+\) = .*/\1/p')
if [ "$local_kernel_version" = "$release_kernel_version" ] && [ -z "$git_password" ] && [ "$(whoami)" != "sbwml" ]; then
    git clone https://$github/jianyun8023/target_linux_generic -b openwrt-24.10 target/linux/generic-6.12 --depth=1
else
